...
class AbstractStructure(ABC):
    ...
    # нотация @abstractmethod указывает дочернему классу какие методы необходимо реализовать,
    # т.е. наполнить фактическим содержимым (некоторыми конкретными алгоритмами)
